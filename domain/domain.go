...
func (do *Domain) reload() error {
	...
	for {
		...
		log.Warnf("[ddl] load schema, ver:%v, time:%v", ver.Ver, time.Now())
		if err == nil {
	...
	for {
		...
		case <-timer.C:
			...
			if sub > lease {
				...
				do.SchemaValidity.SetValidity(false, 0)
			} else {
			...
			log.Warnf("loop, sub:%v, lease:%v, succ:%v, waitTime:%v, now:%v", sub, lease, lastReloadTime, waitTime, time.Now())
